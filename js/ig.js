    ...
    if(!calculator.globalVariable.tutorial.weAreInTutorial) {
        ...
        if(calculator.globalVariable.isIGA) {
            ...
            if(calculator.globalVariable.subjectiveIndex === -1) {
                ...
                $('.bswLeft').css({'background-color':'rgb(35, 79, 30)', 'color':'white'});
                $('.bswRight').css({'background-color':'rgb(60,60,60)', 'color':'white'});
...
calculator.graphics.update.IG_efficiencyBar = function(efficiency1, efficiency2) {
    ...
    if((effi1 / effi2) > 1) {
        ...
        var myText = (val1 >= 0.99) ? '100+' : (effi1 / effi2).toFixed(2);
    ...
    if((effi1 / effi2) < 1) {
        ...
        var myText2 = (val2 >= 0.99) ? '100+' : (effi2 / effi1).toFixed(2);
...
$('.IG_inputL').hover(
    ...
    function() {
        ...
        $('.IG_decisionWrapL').css({'transition':'0.3s', 'transform':'scale(1)'});
...
$('#IG_olSlider1').hover(
    function() {
        ...
        $('.IG_activeFollowerRight').css({'opacity':'1'});
        $('.IG_passiveFollowerRight').css({'opacity':'0'});
    ...
    if(calculator.globalVariable.isIGA) {
        ...
        if(calculator.globalVariable.tutorial.weAreInTutorial) {
            ...
            if(calculator.globalVariable.tutorial.IGSameColor) {
                ...
                $('.IG_contestTitle22, .IG_contestTitle2').css({'color':'white',
...
calculator.icons.update.IG_leaderSize = function() {
    ...
    calculator.icons.set.size('IG_splc1', 85 * m1, 65 * m1);
    calculator.icons.set.size('IG_splc2', 85 * m2, 65 * m2)
...
calculator.lock.IG_activate = function(array) {
    ...
    $('.IG_lockedCover_l1').css({'z-index' : l1});
    $('.IG_lockedCover_l2').css({'z-index' : l2});


    // readjust the question marks to not be displayed if it is locked
    $('.IG_sliderQuestion_l1').css({'opacity': (1 - array[0]).toString()});
    $('.IG_sliderQuestion_l2').css({'opacity': (1 - array[1]).toString()});
    ...
    if(calculator.globalVariable.subjectiveView) {
        ...
        if(calculator.globalVariable.subjectiveIndex === 0) {
            ...
            $('.IG_lockedCover_l1').css({'z-index' : '-1'});
        }

        if(calculator.globalVariable.subjectiveIndex === 1) {
            $('.IG_sliderQuestion_l2').css({'opacity': '0'});
            $('.IG_lockedCover_l2').css({'z-index' : '-1'});
        }
...
calculator.roll.IG_l1 = function(index, prevIndex, textArray) {
    ...
    if(!calculator.stopRolling) {
        ...
        setTimeout(()=>calculator.roll.IG_l1(nextIndex, prevIndex, textArray), calculator.roll.displayTime);
...
calculator.roll.IG_reset = function() {
    ...
    setTimeout(()=>calculator.roll.IG_initiate(), calculator.roll.displayTime);
...
calculator.space.update.IG_heights = function() {
    ...
    var c = calculator.space.IG_contestIsOpen ? 170 : 0;
    var hs = calculator.space.IG_hsIsOpen ? 295 : 0; // this may be smaller depending on the size of the icons
    var cr = calculator.space.IG_contestResultsIsOpen ? 150 : 0;//145 OLD VALUE
    ...
    $('.IG_generalMarginBox').css({'transition':'0.3521s', 'height' : total});
...
calculator.results.show.IG_outcomes = function() {
    ...
    if(calculator.globalVariable.display.IG_hsIcons) {
        ...
        // even if we need it
        calculator.section.hs.opacity.IG_SFiALiFiS([1,1,1,1,0,0]);
...
calculator.results.show.IG_leaderOutcomes = function() {
        ...
        $('.IG_payoffWrap, .IG_imageWrap23').css({'transition' : '0.15s', 'opacity':'1'});
        calculator.space.IG_contestResultsIsOpen = true;
...
calculator.section.all.IG_opacity = function(opt) {
    ...
    $('.IG_generalMarginBox').css({'opacity' : opt.toString()});
...
calculator.section.power.display.IG_barText = function() {

    $('.IG_powerRatio').css({'margin-top':'0px', 'margin-bottom':'-4px'});
...
IG_spin2.onclick = function() {
    if(calculator.globalVariable.IG_bottomSpinButtonIsEnabled) {
        ...
        calculator.titles.hs.ghost.IG_hide();
