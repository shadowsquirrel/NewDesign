...
window.onload = function() {
    ...
    // Recall that only for the feedback stages where we have this spin stuff
    // going on norp listener only updates the number of ready player
    // and norp becomes only relevant only for the players who are waiting
    // for those who are still doing stuff they will not see the norp graphics
    // once norp = 6 the game will automatically proceed to the next stage
    //
    ...
    var generateFinalPayoff = function() {
        ...
        final.setRound = function() {
            ...
            $('#someRound').html(mainData.myRound);
            setTimeout(()=>{
        ...
        final.determineSkipTuto = function() {

            if(mainData.myRound > 1) {
                final.skipTuto = 1;
            } else {
                final.skipTuto = 0;
            }
        ...
        final.assignFinalPayoffs(mainData.payoffs);
        ...
        $('.iwIndex1').hover(
            function() {
                ...
                if(final.globalVariable.blockerList[0]) {
                    ...
                    data = mainData.payoffs[group][role][1];

                    final.set.og1NP(data);

                    data = mainData.payoffs[group][role][2];
                    ...
                    if(final.globalVariable.wlx === 0) {
                        ...
                        data = mainData.payoffs[group][role][4];

                    } else {

                        data = mainData.payoffs[group][role][3];
        ...
        $('.iwIndex2').hover(
            function() {
                ...
                if(final.globalVariable.blockerList[1]) {
                    ...
                    // stage 2 & 3
                    data = mainData.payoffs[group][role][0];
        ...
        $('#doneWithRound').click(function() {
            ...
            if(final.globalVariable.doneButtonActive) {
                ...
                // ----------------------------------------- //
                //
                // var stage = 'finalPayoff';
                // node.emit('HTML-ready', stage);
                console.log('node.emit(HTML-ready, stage = finalPayoff)');

                setMetaNorp(168, -45, 0.55);
                showNorp(1);
                flowLoad(1);

                $('.doneWithRoundButton').css({'transition':'1s',
                'transform':'scale(0)', 'opacity':'0'});

                $('.someTitle').css({'transform':'scale(1.5)', 'margin-top':'125px'});
