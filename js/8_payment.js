...
debug.start.P = function(treatment) {
    ...
    setTimeout(()=>{
        ...
        $('.og1MoneyBagWrap1, .og1MoneyBagWrap2, .igTopMoneyBagWrap1, .igBottomMoneyBagWrap1, .og2MoneyBagWrap1, .og2MoneyBagWrap2').css({'display':'flex', 'z-index':'2'});

        setTimeout(()=>{
            map.opacity.section([1,0,1], 1.5);
            $('.og1MoneyBagWrap1, .og1MoneyBagWrap2, .igTopMoneyBagWrap1, .igBottomMoneyBagWrap1, .og2MoneyBagWrap1, .og2MoneyBagWrap2').css({'transition':'1.5s', 'opacity':'1'});
            $('.section1Text, .section3Text').css({'transition':'1s', 'color':'green', 'font-weight':'700'});
        }, 2000)
...
$('#btn-P-6').click(function() {
    ...
    // close the box
    box.transition('P-6', 'P-601', 0, 0, 750);


    setTimeout(()=>{
        $('.moneyDistributionWrap').css({'margin-top':'-1px', 'margin-bottom':'32px'});
        setTimeout(()=>{
            $('.moneyDistributionWrap').css({'opacity':'1'});
        }, 350)
    }, 2500)
    ...
    // show the new info OK button
    setTimeout(()=>{
        box.button.show('P-601');
    }, 4000);

})

$('#btn-P-601').click(function() {

    // close the box
    box.transition('P-601', 'P-602', 0, 0, 750);

    setTimeout(()=>{
        $('.noMoney').css({'transition':'1s', 'transform':'scale(0)'});
        setTimeout(()=>{
            $('.bigCrownIcon').css({'transition':'1s', 'transform':'scale(1)'});
        }, 750)
    }, 750)
    ...
    // show the new info OK button
    setTimeout(()=>{
        box.button.show('P-602');
    }, 3000);

})

$('#btn-P-602').click(function() {

    // close the box
    box.transition('P-602', '', 0, 0, 0);

    $('.noMoney, .bigCrownIcon, .moneyBag').css({'transition':'0.5s', 'opacity':'0'});
    map.opacity.section([0,0,0]);

    setTimeout(()=>{
        $('.newRound').css({'transition':'0s', 'margin-top':'-270px', 'margin-left':'35px'});
        setTimeout(()=>{
            map.animate.showPayoffs2(1000)
        }, 100)
        setTimeout(()=>{
            $('.thisIsExampleWrap').css({'opacity':'1'})
        }, 400)
...
$('#btn-P-8').click(function() {
    ...
    setTimeout(()=>{
        // map.wheel.setRoundResult(1);
        map.wheel.setRoundResultDynamic(1, 1500, 600, 20);
        $('.netPayoffWheelPickedResult').css({'display':'block'});
    },2000)


    setTimeout(()=>{
        box.transition('', 'P-9', 0, 0, 1);
    }, 4000);
...
$('#btn-P-10').click(function() {
    ...
    // close the box
    box.transition('P-10', '', 0, 0, 1);

    $('.netPayoffTextAndWheel').css({'transition':'0.5s', 'transform':'scale(0.7)', 'margin-bottom':'-72px', 'margin-top':'-88px'});
    $('.netPayoffWheelPickedResult').css({'transition':'0.5s', 'margin-top':'20px', 'margin-bottom':'-30px'});

    map.wheel.wheelSpinWinnerPredetermined = true;
    map.wheel.predeterminedWinner = 8;
...
$('#btn-P-11').click(function() {
    ...
    map.wheel.wheelSpinWinnerPredetermined = true;
    map.wheel.predeterminedWinner = 2;


    map.wheel.spin(1, 10);

    // close the box
    box.transition('P-11', 'P-12', 0, 0, 1);
...
$('#btn-P-12').click(function() {
    ...
    map.wheel.wheelSpinWinnerPredetermined = true;
    map.wheel.predeterminedWinner = 5;

    map.wheel.spin(1, 10);

    // close the box
    box.transition('P-12', 'P-14', 0, 0, 1);

    setTimeout(()=>{
        box.button.show('P-14');
    }, 2000)

})

$('#btn-P-14').click(function() {
